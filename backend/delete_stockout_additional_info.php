...
if(!hasStockOutAdditionalInfoDeleteAccess($mysqli)){
    exit("Access denied. You do not have permission to delete stock out additional information.");
}
...
$deleteStmt = $mysqli->prepare("DELETE FROM stockout_additional_information WHERE id = ? LIMIT 1");
if(!$deleteStmt){
    exit("SQL Error: " . $mysqli->error);
}
...
$actionType = ($stockoutType === "server") ? "DELETE SERVER STOCK OUT INFO" : "DELETE ASSET STOCK OUT INFO";

$description = "User [$username] deleted additional information from a $stockoutType stock out record.\n"
    ...
    . "Deleted Information: {$info['additional_information']}\n"
    . "IP Address: $ip\n"
    . "Time: $time";
